...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, Bottleneck_AM, Bottleneck_AM2, C3, C3TR, C3SPP, C3Ghost, CBAM, C3AM, C3AM2, BoT3,CoT3,
                 C3AM3, SAM, CAM, PAM, Reshape, TransformerBlock,CBH, ES_Bottleneck]:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in [BottleneckCSP, C3, C3TR, C3Ghost, C3AM , C3AM2, C3AM3, TransformerBlock,BoT3,CoT3]:
                args.insert(2, n)  # number of repeats
                n = 1
        elif m is nn.BatchNorm2d:
            args = [ch[f]]
        elif m in [Concat, Concat3]:
        ...
        # 添加注意力机制
            ...
            c1, c2 = ch[f], args[0]
            if c2 != no:
                c2 = make_divisible(c2 * gw, 8)*2 # 这里对SEAttention扩2倍
        elif m is GAMAttention:
            c1, c2 = ch[f], args[0]
            if c2 != no:
                c2 = make_divisible(c2 * gw, 8)*2
