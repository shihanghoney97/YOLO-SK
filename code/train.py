...
from models.common import Concat3
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    # Loggers
    if RANK in [-1, 0]:
        LOGGER.info(f'{save_dir}-{weights}-{opt}-{hyp}-{LOGGER}')
    ...
    for v in model.modules():
        ...
        if isinstance(v, nn.BatchNorm2d):  # weight (no decay)
            ...
            g1.append(v.weight)
        elif isinstance(v, Concat3) and hasattr(v, 'w') and isinstance(v.w, nn.Parameter):
            g1.append(v.w)
    ...
    LOGGER.info(f"{colorstr('optimizer-优化器:')} {type(optimizer).__name__} with parameter groups "
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in [-1, 0]:
            pbar = tqdm(pbar,disable=True, total=nb, ncols=NCOLS, bar_format='{l_bar}{bar:10}{r_bar}{bar:-10b}')  # progress bar
        optimizer.zero_grad()
...
def parse_opt(known=False):
    ...
    parser.add_argument('--half', action='store_true', help='useacmix')
    parser.add_argument('--weights', type=str, default='', help='initial weights path')
    parser.add_argument('--cfg', type=str, default='yolov5s_3SK_FPN_SIoU_Ghost.yaml', help='model.yaml path')
