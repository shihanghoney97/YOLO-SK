...
def set_logging(name=None, verbose=True):
    ...
    # 将日志输出到指定文件中 logs/年月日 时分秒.log
    log_name = datetime.datetime.strftime(datetime.datetime.now(),'%Y-%m-%d_%H-%M-%S')
    logging.basicConfig(format="%(asctime)s-%(message)s", filename='logs/%s.log'%log_name, level=logging.INFO if (verbose and rank in (-1, 0)) else logging.WARNING)
...
# add nms by lq 2022-09-14
# 修改为SIOU  modify by lq 2022-11-05
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # modified by lq  修改NMS调用方式 2022-09-14
